...
assName="af-card list-card"><div className="card-head"><h2>Produtos mais procurados</h2><Link href="/app/mais-procurados">Ver todos</Link></div><div className="empty-list"><Search/><p>Ainda não há dados de procura.</p></div></section><section className="af-card list-card"><div className="card-head"><h2>Oportunidades recentes</h2><Link href="/app/oportunidades">Ver todas</Link></div><div className="empty-list"><Target/><p>Nenhuma oportunidade recente.</p></div></section></div><section className="region-banner"><MapPin/><div><h2>Encontre fornecedores na sua região</h2><p>Mais agilidade para o seu negócio.</p></div><Link href="/app/perto-de-mim" className="af-btn af-btn-primary">Explorar fornecedores</Link></section></main><nav className="bottom-nav">{[[Home,"Início"],[Search,"Buscar"],[MapPin,"Perto de mim"],[Flame,"Mais procurados"],[Package,"Categorias"]].map(([Icon,t],i)=>{const C=Icon as typeof Home;return <Link href="#" className={i===0?"active":""} key={String(t)}><C/><span>{String(t)}</span></Link>})}</nav></div>}
