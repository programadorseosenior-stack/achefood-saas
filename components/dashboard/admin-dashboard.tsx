...
const moduleHref: Record<string,string> = {Dashboard:"/admin/dashboard",Empresas:"/admin/empresas",Usuários:"/admin/usuarios",Produtos:"/admin/produtos",Planos:"/admin/planos",Relatórios:"/admin/relatorios"};
const chart = [{d:"22/04",v:0},{d:"29/04",v:0},{d:"06/05",v:0},{d:"13/05",v:0},{d:"20/05",v:0}];
const tones = ["orange","green","blue","purple"];
const quickActions = [[Building2,"Gerenciar empresas","/admin/empresas"],[Users,"Gerenciar usuários","/admin/usuarios"],[FileCheck2,"Aprovar produtos","/admin/produtos"],[ShieldCheck,"Gerenciar planos","/admin/planos"],[CircleDollarSign,"Relatórios financeiros","/admin/relatorios"],[Settings,"Configurações da plataforma","/admin/dashboard#configuracoes"]] as const;
...
export function AdminDashboard({adminName,metrics}:Props){
 ...
 return <div className="admin-shell">
  <aside className={drawer?"admin-side open":"admin-side"}><AcheFoodLogo/><nav>{nav.map(([Icon,label],i)=><Link key={label} className={i===0?"active":""} href={moduleHref[label]||`/admin/dashboard#${label.toLowerCase()}`} onClick={()=>setDrawer(false)}><Icon/>{label}</Link>)}</nav><button onClick={logout}><LogOut/>Sair</button></aside>
  ...
  <nav className="admin-bottom"><Link className="active" href="/admin/dashboard"><LayoutGrid/>Dashboard</Link><Link href="/admin/empresas"><Building2/>Empresas</Link><Link href="/admin/usuarios"><Users/>Usuários</Link><Link href="/admin/relatorios"><Gauge/>Relatórios</Link><Link href="/admin/planos"><Menu/>Mais</Link></nav>
 </div>
}
